...
import type { CourseRunStageStatus } from "@/types/seaca";
...
const statusClasses: Record<CourseRunStageStatus, string> = {
  idle: "bg-[#f2ece4] text-[#8d8172]",
  running: "bg-[#eff8e9] text-[#4f8938]",
  completed: "bg-[#eff8e9] text-[#4f8938]",
...
export function CoursePreviewGrid({ pages }: CoursePreviewGridProps) {
  ...
  if (orderedPages.length === 0) {
    return (
      <section
        aria-label="课程多页预览"
        className="rounded-[24px] border border-[#e6ddd1] bg-[#fffdf8] p-6 text-center text-sm text-[#988e80]"
      >
        课程页面生成后，可以在这里统一预览。
      </section>
    );
  ...
  return (
    <section
      ...
      className="rounded-[24px] border border-[#e6ddd1] bg-[#fffdf8] p-4 shadow-[0_12px_36px_-30px_rgba(56,44,25,0.5)] sm:p-5"
    >
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <p aria-live="polite" className="text-xs text-[#8d8172]">
      ...
      <div className="mt-4 grid min-h-0 gap-4">
          ...
          {orderedPages.map((page, index) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "flex min-h-16 w-full items-center gap-3 rounded-2xl border px-3 py-2.5 text-left outline-none transition-colors focus-visible:ring-2 focus-visible:ring-[#77cc57] focus-visible:ring-offset-2",
                  selected
                    ? "border-[#a8d697] bg-[#f1f9ed]"
                    : "border-[#e6ddd1] bg-[#fffdf8] hover:bg-[#f8f3ec]",
                )}
                ...
                type="button"
              >
                <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-white font-mono text-xs font-semibold text-[#5d9845] shadow-sm">
                  {String(page.order).padStart(2, "0")}
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-sm font-semibold text-[#4c3e2b]">
                    {page.title}
                  </span>
                  <span
                    className={cn(
        ...
        <div
          ...
          className="min-w-0 rounded-2xl border border-[#e6ddd1] bg-[#fcf9f2] p-3 sm:p-4"
          id={panelId}
          role="tabpanel"
          tabIndex={0}
        >
          {selectedPage.status === "completed" && selectedPage.htmlOutput ? (
            ...
            <div
              aria-live="polite"
              className="flex min-h-[220px] items-center justify-center rounded-2xl bg-[#f8f3ec] px-6 text-center text-sm leading-6 text-[#8d8172]"
            >
