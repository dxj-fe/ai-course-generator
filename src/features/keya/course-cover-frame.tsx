...
export function CourseCoverFrame({
  ...
  return (
    <span
      aria-hidden="true"
      className={cn(
        "absolute inset-0 overflow-hidden bg-[linear-gradient(135deg,#eaf3e7,#fff0c9_55%,#f3e7ce)]",
        className,
      )}
      ref={containerRef}
    >
      <span className="absolute -right-8 -top-10 size-32 rounded-full border-[20px] border-white/35" />
      <span className="absolute inset-x-5 bottom-5 line-clamp-2 text-xl font-semibold leading-7 text-[#2d332b]">
        {title}
      </span>
      ...
      {coverUrl ? (
        <iframe
          aria-hidden="true"
          className={cn(
            "pointer-events-none absolute block h-[540px] w-[960px] origin-top-left border-0 bg-white transition-opacity duration-200",
