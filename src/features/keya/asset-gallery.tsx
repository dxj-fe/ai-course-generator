...
export function AssetGallery({ results }: { results: AssetGenerationResult[] }) {
  return (
    <ul className="mt-3 grid grid-cols-2 gap-3">
      {results.map((result) => (
        <li
          className="min-w-0 overflow-hidden rounded-xl border border-[#e8dfd0] bg-[#fffcf5]"
        ...
        >
          {result.status === "ready" && result.asset?.uri ? (
            ...
            <div className="flex aspect-[4/3] items-center justify-center bg-[repeating-linear-gradient(135deg,#f4efe7_0,#f4efe7_8px,#faf7f1_8px,#faf7f1_16px)] px-3 text-center text-xs leading-5 text-[#7a7468]">
              {result.fallback?.description ?? "素材已使用安全降级"}
            </div>
          )}
          <div className="p-3">
            <div className="flex flex-wrap items-center gap-2">
              <Badge className="h-auto rounded-full border-0 bg-[#edf5ee] px-2 py-0.5 text-[10px] text-[#397a52]">
