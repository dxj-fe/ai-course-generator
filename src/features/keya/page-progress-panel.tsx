...
export function PageProgressPanel({ run }: PageProgressPanelProps) {
  ...
  return (
    <section
      ...
      className="rounded-[20px] border border-[#e8dfd0] bg-[#fffcf5] p-4 sm:p-5"
    >
      <div>
      ...
      {pages.length > 0 && run ? (
        <ol aria-label="课程逐页生成进度" className="mt-4 grid gap-3">
          {pages.map((page) => {
            ...
            return (
              <li
                className="rounded-2xl border border-[#f1e7d5] bg-[#fdfaf5] p-3 sm:p-4"
                key={page.id}
              >
                <div className="flex min-w-0 flex-wrap items-start justify-between gap-2">
                  <div className="flex min-w-0 items-start gap-2.5">
                    ...
                    <div className="min-w-0">
                      <h4 className="text-sm font-semibold leading-5 text-[#3f4a40]">
                        {page.title}
                      </h4>
        ...
        <p className="mt-4 rounded-2xl bg-[#f6eedc] px-4 py-5 text-center text-sm leading-6 text-[#7a7468]">
...
function StageProgress({
  ...
  return (
    <div className="min-w-0 rounded-xl bg-white px-3 py-2.5">
      <dt className="text-[11px] font-medium text-[#7a7468]">{label}</dt>
      <dd className="mt-1.5">
