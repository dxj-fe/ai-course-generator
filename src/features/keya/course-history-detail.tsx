...
export function CourseHistoryDetail({ courseId }: { courseId: string }) {
  ...
  if (!detail) return <div aria-live="polite" className="flex min-h-[55vh] items-center justify-center gap-2 text-sm text-[#7a7468]"><LoaderCircle className="size-4 animate-spin" /> 正在加载持久化课程…</div>;
  ...
  return (
    <main className="min-h-[calc(100vh-64px)] bg-[#fff9ee] pb-16 text-[#2d332b]">
      <div className="mx-auto w-[calc(100%-32px)] max-w-[1200px] pt-7 sm:w-[calc(100%-48px)]">
        ...
        <header className="mt-5 rounded-[26px] border border-[#e8dfd0] bg-[#fffcf5] p-5 sm:p-7">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="max-w-3xl">
              <Badge className="border-0 bg-[#edf5ee] text-[#2f6845]">{course.status === "completed" ? "已完成" : course.status === "running" ? "生成中" : course.status === "failed" ? "生成失败" : "已取消"}</Badge>
              <h1 className="mt-3 text-2xl font-semibold sm:text-3xl">{course.intent?.topic ?? course.userPrompt}</h1>
              <p className="mt-3 text-sm leading-6 text-[#7a7468]">{course.outline?.overview ?? course.userPrompt}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <Button className="rounded-full bg-[#397a52] text-white hover:bg-[#2f6845]" disabled={course.status !== "completed" || exporting} onClick={handleExport}>
          ...
          <dl className="mt-6 grid gap-3 border-t border-[#f1e7d5] pt-5 text-sm sm:grid-cols-4">
        ...
        <section aria-labelledby="run-history-title" className="mt-6 rounded-[24px] border border-[#e8dfd0] bg-[#fffcf5] p-5">
          <h2 id="run-history-title" className="text-lg font-semibold">运行记录</h2>
          {detail.runs.length ? <ol className="mt-4 grid gap-3">{detail.runs.map((run) => <li className="rounded-2xl bg-[#f6eedc] px-4 py-3 text-sm" key={run.taskId}><div className="flex flex-wrap justify-between gap-2"><span className="font-medium">{run.source === "langgraph" ? "LangGraph" : "Workflow"} · {run.status}</span><time className="text-[#7a7468]">{formatDate(run.createdAt)}</time></div>{run.error ? <p className="mt-2 text-[#a44f3d]">{run.error.message}</p> : null}</li>)}</ol> : <p className="mt-3 text-sm text-[#7a7468]">没有关联的任务记录。</p>}
        </section>
      </div>
    </main>
  );
...
function Meta({ label, value }: { label: string; value: string }) { return <div><dt className="text-xs text-[#7a7468]">{label}</dt><dd className="mt-1 font-medium">{value}</dd></div>; }
function DetailError({ message, onRetry }: { message: string; onRetry(): void }) { return <main className="flex min-h-[calc(100vh-64px)] items-center justify-center bg-[#fff9ee] px-6 text-center text-[#2d332b]" role="alert"><div><TriangleAlert className="mx-auto size-9 text-[#b65e49]" /><h1 className="mt-3 font-semibold">课程无法打开</h1><p className="mt-2 text-sm text-[#7a7468]">{message}</p><Button className="mt-4 rounded-full" onClick={onRetry} variant="outline"><RefreshCw />重试</Button></div></main>; }
