...
import { AiRequestError, AiSchemaValidationError } from "@/server/ai/error";
import {
...
export type ParseUploadedFileContext = {
  abortSignal?: AbortSignal;
  traceId: string;
};

export type ParseUploadedFileDependencies = {
...
const defaultDependencies: ParseUploadedFileDependencies = {
  extractPdfText,
  summarize: summarizeReference,
};

/** 将单份小型用户资料转换为可校验、可引用的 Reference Pack。 */
export async function parseUploadedFileSkill(
  file: File,
  context: ParseUploadedFileContext,
  overrides: Partial<ParseUploadedFileDependencies> = {},
...
async function extractText(
  ...
  bytes: Uint8Array,
  pdfExtractor: ParseUploadedFileDependencies["extractPdfText"],
) {
  if (sourceType === "pdf") {
    try {
