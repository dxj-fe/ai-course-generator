    ...
    const state = await runPageQAModelStep(
      {
        ...
        courseContext: {
          ...
          facts: input.execution.architecture.coursePack.facts.map(
            ({ id, text }) => ({ id, text }),
          ),
          terms: input.execution.architecture.coursePack.terms.map(
    ...
    execution.localResourceSession?.loadedResources.filter(
      ({ logicalPath }) =>
        logicalPath.startsWith(
          "agent/skills/course-page-design/references/",
        ),
    ) ?? []
  );
}
