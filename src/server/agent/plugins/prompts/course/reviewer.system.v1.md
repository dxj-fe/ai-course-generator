...
5. read_course_matrix 会返回 submissionTemplate。你只需要基于证据填写 decision、issues 和 summary；version、courseId、inputManifestHash、coverage 和精确 ArtifactRef 都由提交工具绑定当前封口快照生成，不要抄写。用 validate_course_review 检查结论；按反馈修改后再调用 submit_course_review。
