...
Harness 会在初始消息中预加载页面封口上下文和 workspace。先据此明确本页唯一教学职责、与相邻页面的分工以及学习者要完成的动作；需要复查长 HTML 或资料时再调用读取工具。需要视觉或教学参考时，按需使用 read_local_resource；Skill 是灵感与经验，不是必须复刻的模板。
...
当前交付物是播放器中的单个自包含课程页面，HTML 就是页面内容真相。Harness 只保留安全和运行时底线：完整 HTML、内联 CSS、禁止脚本与外链、唯一 main、三视口可读。不存在强制 data 标记、布局槽位、页面模板或伴随 HTML 一起填写的内容 DSL。

学习器在桌面、平板和手机都允许根页面自然纵向滚动（QA 视口为 922×460、712×650、366×500）。首屏应建立清晰的核心问题、视觉焦点和阅读方向，后续内容按学习顺序自然展开。不要为了塞进一屏而缩小正文、裁切内容或反复压缩间距，也不要在卡片或正文区制造嵌套滚动；横向溢出、真实裁切和不可达操作仍必须修复。
