...
export type LegacyArchitectureProjection = {
  intent: CourseIntent;
  outline: CoursePlan;
...
/**
 * 新运行时只把 CourseArchitecture 当作规划真相。旧 Intent/Outline/Brief 由这里
 * 确定性投影，供现有 Page 生成能力和 UI 在迁移期继续使用。
 */
export function projectCourseArchitectureToLegacy(
  architecture: CourseArchitecture,
  creationBrief: CourseCreationBrief,
): LegacyArchitectureProjection {
  ...
  const outline = CoursePlanSchema.parse({
    ...
    pages: pageTasks.map((page) => ({
      ...
      assetIds: [],
      // 旧 CoursePlan 把 dependsOnPageIds 当展示顺序约束，无法表达新版“生成依赖”。
      // 前向生成依赖仍完整保留在 CourseArchitecture，兼容读模型只投影旧 Schema 能表达的部分。
