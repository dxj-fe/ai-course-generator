import {
  runPedagogyAgent,
  type PedagogyAgentState,
} from "@/server/agents/pedagogy-agent";
import {
  runStoryAgent,
  type StoryAgentState,
} from "@/server/agents/story-agent";
import {
  runVisualDirectorAgent,
  type VisualDirectorAgentState,
} from "@/server/agents/visual-director-agent";
import type {
  AgentEvent,
  AgentRuntimeContext,
...
export type CourseDesignAgentName =
  | "pedagogy"
  | "story"
  | "visual";

export type CourseDesignEvent = AgentEvent & {
  agent: CourseDesignAgentName;
};
...
export type CourseDesignWorkflowState = {
  ...
  error?: {
    agent: CourseDesignAgentName | "workflow";
    code: string;
    message: string;
  };
};
...
export type CourseDesignWorkflowDependencies = {
  ...
  ): Promise<PedagogyAgentState>;
  runStory(
    input: {
      intent: CourseIntent;
      outline: CoursePlan;
      pedagogy: NonNullable<PedagogyAgentState["plan"]>;
    },
    context: AgentRuntimeContext,
  ): Promise<StoryAgentState>;
  runVisual(
    input: {
      intent: CourseIntent;
      outline: CoursePlan;
      pedagogy: NonNullable<PedagogyAgentState["plan"]>;
      story: NonNullable<StoryAgentState["arc"]>;
    },
    context: AgentRuntimeContext,
  ): Promise<VisualDirectorAgentState>;
};

const defaultDependencies: CourseDesignWorkflowDependencies = {
  runPedagogy: runPedagogyAgent,
  runStory: runStoryAgent,
  runVisual: runVisualDirectorAgent,
};
...
export async function runCourseDesignWorkflow(
  ...
  appendAgentEvents(events, "pedagogy", pedagogyState.events);
  ...
  appendAgentEvents(events, "story", storyState.events);
  ...
  appendAgentEvents(events, "visual", visualState.events);
...
/** 为每条公开事件补充 Agent 来源，并重新编号为一条全局时间线。 */
...
/** 将单 Agent 失败统一映射为前端可展示的工作流错误。 */
function failedState(
  events: CourseDesignEvent[],
  agent: CourseDesignAgentName,
  error: PedagogyAgentState["error"],
  ...
  return {
    ...
    error: {
      ...
      message: error?.message ?? `${agent} Agent 未生成有效结果。`,
    },
  };
}
