...
function openDatabase(databasePath: string, migrateLegacy: boolean) {
  ...
  migrateCourseArtifactDeduplicationScope(database);
...
/**
 * 历史版本只有进程内 courseClaims。启动时为仍处于活动态的有效任务补齐数据库
 * claim；已有有效 claim 不改写，避免另一个进程正在执行时短暂放开课程写权限。
 */
 ...
 * 终态化或损坏时清理孤儿 intent，避免历史行长期污染控制面。
 */
...
/**
 * 早期 agent-v2 表把相同内容跨 WorkOrder 去重，导致修订工作单复用旧产物并丢失
 * 来源关系。现在只在同一 WorkOrder 内去重；不同工作单即使内容没变，也会形成新的
 * 不可变 Artifact 版本。
 */
function migrateCourseArtifactDeduplicationScope(database: DatabaseSync) {
  const uniqueIndexes = database
    .prepare("PRAGMA index_list('course_artifacts')")
    .all() as Array<{ name: string; unique: number }>;
  const hasLegacyContentConstraint = uniqueIndexes
    .filter(({ unique }) => unique === 1)
    .some(({ name }) => {
      const columns = database
        .prepare(`PRAGMA index_info('${name.replaceAll("'", "''")}')`)
        .all() as Array<{ name: string }>;
      return (
        columns.map(({ name: columnName }) => columnName).join(",") ===
        "task_id,course_id,scope_key,kind,content_hash"
      );
    });
  if (!hasLegacyContentConstraint) return;

  runInTransaction(database, () => {
    database.exec(`
      ALTER TABLE course_artifacts
        RENAME TO course_artifacts_legacy_content_dedupe;
      ...
      SELECT
        id, task_id, course_id, page_id, scope_key, kind, version,
        content_hash, created_by_work_order_id, payload, created_at
      FROM course_artifacts_legacy_content_dedupe;

      DROP TABLE course_artifacts_legacy_content_dedupe;
