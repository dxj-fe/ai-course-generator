...
方案已完成第二期落地：12 个生产参考拥有统一画像，确定性匹配器同时驱动 `/templates` 与 Page Creator 的视觉参考包；Harness 为并行页面预加载一个主方向、两个备选方向和精确配方路径。34 个 bold 配方继续作为按需读取的参考库，不会因为文件存在就变成固定布局合同。
...
本项目借用网页演示的视觉语言和验收方法，但不复制多页 deck wrapper、导航器或作者脚本。单个课程页面使用 Keya 自己的 1920×1080 固定舞台，由播放器运行时同比缩放；HTML 是内容真相，不要求作者输出布局 DSL。
...
以上 8 个核心方向加 `neo-grid-bold`、`soft-editorial`、`cartesian`、`studio-electric` 四个增强方向，共 12 个可直接进入匹配合同的生产参考。其他预设和 bold 配方继续按需提供设计语言，不能因为文件存在就变成布局模板。
...
- 12 个生产参考已经补齐家族、受众、学习动作、叙事模式、内容承载、正式度、风险场景和 recipe path。
