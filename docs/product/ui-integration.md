...
| 模板浏览与按需视觉匹配 | `/templates` | 模板目录中的匹配实验室、三方向解释与完整能力谱系 |
| 引用上传 | `/chat` | 对话输入附件 |
...
- 课程播放器默认收起缩略图，把 16:9 主画布作为最大视觉层级；HTML 在固定舞台内同比缩放，播放器不提供横向或纵向滚动。
- 样式复用 Keya 组件与现有设计变量，避免引入独立视觉系统。
- 视觉模板匹配复用 `src/shared/templates/style` 的确定性画像与评分结果；课程 Architect 与 `/templates` 必须使用同一套逻辑，避免展示推荐和实际生成分叉。
