...
- Keya `/chat` 逐页 HTML 状态、公开 Agent Timeline、错误重试和右侧安全预览。
...
右侧工作区适合快速检查单页结果，但 390px 面板不足以验收桌面构图。独立路由提供更接近课程成品的画布，并保留 Keya 的顶部课程栏和底部页码结构。当前没有后端课程持久化，所以 HTML 不放进 URL，也不伪造永久链接；控制器把已校验文档写入带随机 ID 的临时浏览器缓存，新路由读取后再次验证。
