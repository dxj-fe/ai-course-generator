...
describe("course generation adapter", () => {
  it("maps the current batch attempt into existing Keya stages", () => {
    ...
    const run = projectCourseStateToKeyaRun(response, {
      id: "run-1",
  ...
  it("maps the persisted current stage to a live Keya running stage", () => {
    ...
    const doneRun = projectCourseStateToKeyaRun(
      {
    ...
    const errorRun = projectCourseStateToKeyaRun(
      {
  ...
  it("derives concurrent page stages from each worker-local state", () => {
    ...
    const run = projectCourseStateToKeyaRun(
