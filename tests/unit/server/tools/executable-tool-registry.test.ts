...
describe("ExecutableToolRegistry", () => {
  it("registers, lists and executes a tool", async () => {
    const registry = new ExecutableToolRegistry(vi.fn()).register(validTool);
  ...
  it("rejects duplicate tool names", () => {
    ...
    expect(() => registry.register(validTool)).toThrow("Tool 已注册");
  });

  it("rejects invalid input before execute and records a failed event", async () => {
    const eventSink = vi.fn();
    const execute = vi.fn(validTool.execute);
    const registry = new ExecutableToolRegistry(eventSink).register({
      ...validTool,
      execute,
    });
