...
import {
  createImagePromptModelStep,
  createImagePromptModelStepState,
  resolveImagePromptInput,
  validateImagePromptOutput,
} from "../../../../src/server/agent/plugins/model-steps/course/image-prompt-model-step";
import {
  pageContentDsl,
  visualBrief,
...
describe("ImagePromptModelStep", () => {
  ...
  it("skips the model when the page has no asset slots", async () => {
    ...
    const state = await createImagePromptModelStep({ generateDirections }).run(
      createImagePromptModelStepState({ content: pageContentDsl, visualBrief }),
